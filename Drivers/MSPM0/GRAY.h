...
#define Switch_Address_0(i) \
    ((i) ? \
        ...
        DL_GPIO_clearPins(Gray_Address_PORT, Gray_Address_AD0_PIN)) // 地址位0控制

#define Switch_Address_1(i) \
    ((i) ? \
        DL_GPIO_setPins(Gray_Address_PORT, Gray_Address_AD1_PIN) : \
        DL_GPIO_clearPins(Gray_Address_PORT, Gray_Address_AD1_PIN)) // 地址位1控制

#define Switch_Address_2(i) \
    ((i) ? \
        DL_GPIO_setPins(Gray_Address_PORT, Gray_Address_AD2_PIN) : \
        DL_GPIO_clearPins(Gray_Address_PORT, Gray_Address_AD2_PIN)) // 地址位2控制
...
void No_Mcu_Ganv_Sensor_Process(No_MCU_Sensor *sensor);

// 用户接口函数
unsigned char Get_Digtal_For_User(No_MCU_Sensor *sensor); // 获取数字量
unsigned char Get_Normalize_For_User(
    No_MCU_Sensor *sensor,
    unsigned short *result); // 获取归一化值
unsigned char Get_Anolog_Value(
    No_MCU_Sensor *sensor,
